...
impl GenerationLog {
    ...
    pub fn append(
        ...
        if parent_hash.as_ref() != self.head().map(|h| &h.snapshot_hash) {
...
mod tests {
    ...
    fn append_chain() {
        ...
        log.append("h1".to_string(), Some("h0".to_string())).unwrap();
        log.append("h2".to_string(), Some("h1".to_string())).unwrap();
        assert_eq!(log.len(), 3);
    ...
    fn parent_mismatch_rejected() {
        let mut log = GenerationLog::new();
        log.append("h0".to_string(), None).unwrap();
        let result = log.append("h1".to_string(), Some("WRONG".to_string()));
